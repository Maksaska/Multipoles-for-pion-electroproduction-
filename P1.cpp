...
vector<vector<double>> EMS(const double& W, const double& Q2, const double& theta, const int& limiter, const int& id)
{
	...
	vector<double> buff;
	vector<double> G_f; 
	double val, Const; Const = Mpip*1000;
...
int main(int argc, char **argv)
{
	...
	int term_number(6); double Q2, theta;
	vector<vector<double>> Multipoles;
	
	Q2 = fRand(0.3, 1); Q2 = 0.7;
	theta = fRand(0, M_PI); //theta = 70*M_PI/180;
	
	Timer time_period;
	
	cout << "W_GeV" << "," << "Q2_GeV^2" << "," << "A0_10^-3/Mpi+" << "," << "E+" << ",";
	...
	for(double W = 1.1; W < 2.0; W = W + 0.01)
	{
		for(double Q2 = 0.05; Q2 < 5.0; Q2 = Q2 + 0.05)
		{
			cout << W << "," << Q2 << "," << " " << ",";
			Multipoles = EMS(W, Q2, theta, term_number, 0);
			PrintBiggy(Multipoles);
			cout << " " << ",";
			
			Multipoles.clear();
			Multipoles = EMS(W, Q2, theta, term_number, 1);
			PrintBiggy(Multipoles);
			cout << " " << ",";
	...
	cout << "Time elapsed: " << time_period.elapsed() << '\n';
